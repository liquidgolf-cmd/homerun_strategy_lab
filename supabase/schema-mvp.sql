-- Simplified Supabase Database Schema for Homeruns Strategy Lab MVP
-- Module 0 (Current Reality) Only
-- No foreign key constraints - we validate users via JWT tokens

-- Enable UUID extension
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Drop existing tables if they exist (for clean MVP start)
...
-- Module responses table (Module 0 only for MVP)
CREATE TABLE module_responses (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  "userId" UUID NOT NULL, -- Reference to user_sessions.userId (no FK constraint)
  "moduleNumber" INTEGER NOT NULL CHECK ("moduleNumber" = 0), -- Only Module 0 in MVP
