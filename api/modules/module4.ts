/**
 * Vercel Serverless Function: Module 4 Endpoints
 * GET /api/modules/module4 - Get Module 4 response
 * POST /api/modules/module4 - Save Module 4 response
 */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    if (req.method === 'GET') {
      const response = await getModuleResponse(user.id, 4);
      if (!response) {
        return res.status(404).json({ error: 'Module 4 response not found' });
      }
    ...
    if (req.method === 'POST') {
      ...
      if (auditReviewDocument) {
        await updateUserSession(user.id, {
          currentModule: 4, // Stay on module 4 (last module)
          completionStatus: 5, // All 5 modules completed
        });
      }
    ...
    console.error('Error in module4 handler:', error);
